...
qq.FineUploaderBasic.prototype = {
	...
	clearStoredFiles: function () {
		"use strict";
		while (this._storedFileIds.length) {
			this._deleteFile(this._storedFileIds.shift());
		}
	},
	retry: function (id) {
	...
	_createUploadHandler: function () {
		...
		var handler = new qq[handlerClass]({
			...
			delHandler: this._options.request.delHandler,
	...
	_deleteFile: function (file_id) {
		var item = this.getItemByFileId(file_id);

		if (item) {
			var path = qq(this._find(item, 'hiddenInput')).val();
...
qq.FineUploader = function (o) {
	...
	qq.extend(this._options, {
		...
		callbacks: {
			onComplete: function(id, fileName, responseJSON) {
				...
				if (responseJSON.filepath) {
					var filename = responseJSON.filepath.split('/').slice(-1);

					qq(this._find(item, 'file')).setText(this._formatFileName(filename));
					qq(this._find(item, 'hiddenInput')).value = responseJSON.filepath;
				}
			}
		},
		element: null,
		listElement: null,
		hiddenInputName: 'file',
		dragAndDrop: {
			extraDropzones: [],
			hideDropzones: true,
		...
		template: '<div class="qq-uploader span12">' +
			...
			'</div>',
		fileTemplate: '<li>' +
			'<a class="qq-upload-delete close" href="#">{deleteButtonText}</a>' +
			...
			'<input class="qq-hidden-input" type="hidden" name="{hiddenInput}">' +
			'</li>',
		classes: {
	...
	this._options.fileTemplate = this._options.fileTemplate.replace(/\{hiddenInput\}/g, this._options.multiple ? this._options.hiddenInputName + '[]' : this._options.hiddenInputName);
...
qq.extend(qq.FineUploader.prototype, {
	...
	_onComplete: function (id, fileName, result, xhr) {
		...
		if (result.success) {
			...
			this._find(item, 'delete').style.display = "inline-block";

			if (result.filepath) {
				var filename = result.filepath.split('/').slice(-1);

				item.innerHTML += '<div class="clearfix"></div><img src="' + (this._options.resize.width?'/thumb?f=' + result.filepath + '&w=' + this._options.resize.width + '&h=' + this._options.resize.height + '&m=crop':result.filepath) + '">';
				qq(this._find(item, 'file')).setText(this._formatFileName(filename));
				qq(this._find(item, 'hiddenInput')).value = result.filepath;
				qq(this._find(item, 'hiddenInput')).val(result.filepath);
			}
	...
	_addToListUploaded: function (filepath) {
		var fileName = filepath.split('/').slice(-1),
			item = qq.toElement(this._options.fileTemplate);

		item.qqFileId = qq.getUniqueId();

		this._storedFileIds.push(item.qqFileId);
		...
		this._onComplete(item.qqFileId, item, {success: 'success', filepath: filepath});
		this._filesInProgress++;
	},
	_clearList: function () {
	...
	_bindCancelAndRetryEvents: function () {
		...
		this._attach(list, 'click', function (e) {
			...
			if (qq(target).hasClass(self._classes.cancel) || qq(target).hasClass(self._classes.retry) || qq(target).hasClass(self._classes.delete)) {
				qq.preventDefault(e);
				...
				if (qq(target).hasClass(self._classes.cancel)) {
					...
					qq(item).remove();
				} else if (qq(target).hasClass(self._classes.retry)) {
					qq(item).removeClass(self._classes.retryable);
					self.retry(item.qqFileId);
				} else if (qq(target).hasClass(self._classes.delete)) {
					self._deleteFile(item.qqFileId);
				}
			}
		});
	},
	...
	_showDeleteLink: function (item) {
		var deleteLink = this._find(item, 'delete');
...
qq.UploadHandlerAbstract = function (o) {
	...
	this._options = {
		...
		onDelete: function (id, fileName, response, xhr) {
		},
		onCancel: function (id, fileName) {
		},
		onUpload: function (id, fileName, xhr) {
