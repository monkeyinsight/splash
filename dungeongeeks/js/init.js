...
document.body.onload = function() {
	...
	$page.addEventListener('swiperight', openMobile);
	$page.addEventListener('swipeleft', closeMobile);
	$mobileMenu.addEventListener('swipeleft', closeMobile);
	document.querySelector('.mobile-menu .menu-trigger').addEventListener('click', function(e) {
		e.preventDefault();
		closeMobile();
	});
	document.querySelector('.top-nav .menu-trigger').addEventListener('click', function(e) {
		e.preventDefault();
		if (!$page.style.left || parseInt($page.style.left) == 0) {
			openMobile();
		} else {
			closeMobile();
		}
